...
if len(sys.argv) < 3:
    print("   Ça veux 2 arguments: <fichier info> <répertoire de data>\n")
    exit(1) 
else: 
...
ref = pd.read_csv(f"/data/{os.getenv('USER')}/SurgeNMF/surgeNMF/Reference_SBS96_COSMIC_Catalogue_Ordered.txt", sep="\t", header=0, index_col=0)
...
# **** Pour tester le code, choisissez les signatures  
if (num_sigs == 4):
...
for n in range(num_obs):
    lignes_nulle = rng.choice(num_sigs, size=rng.integers(1, num_sigs), replace=False)
